...
# === Main Generation Function (Refactored Workflow) ===
def main():
    """Main entry point for bullet chart visual generation, following the modular workflow."""
    dataset_map = load_all_datasets()
    if not dataset_map:
        print("\n⚠ No datasets loaded. Exiting.")
        return

    # Load chart positions and final chart definitions
    try:
        with open(os.path.join(OUTPUT_DIR, "powerbi_chart_positions.json"), 'r', encoding="utf-8") as f:
            chart_positions_raw = json.load(f)
            chart_positions = chart_positions_raw if isinstance(chart_positions_raw, list) else [chart_positions_raw]
    except Exception as e:
        print(f"❌ Error loading chart positions: {e}")
        return

    try:
        with open(os.path.join(OUTPUT_DIR, "final.json"), 'r', encoding="utf-8") as f:
    ...
    extracted_objects, extracted_vc_objects = {}, {}
    if bullet_style_sample:
        ...
        if extracted_objects or extracted_vc_objects:
            ...
            print(f"\nℹ️ Matching visual config found in '{REFERENCE_TEXT_PATH}', but no expected styling properties.")
    else:
        print(f"\n❌ Warning: No specific Bar Chart (Bullet Style) visual config found in '{REFERENCE_TEXT_PATH}'.")
    ...
    for chart_entry in bullet_charts:
        ...
        if not resolved_category_pbi_column:
            common_category_alternatives = [
                conceptual_category_field, "State", "State/Province", "Segment", "Region", "Country", "City", "Category Name", "Product Category", "Customer Segment", "Product Name", "Type"
            ]
